...
function migrateTaskData(tasks) {
	tasks.forEach((task) => {
		...
		if (task.color && !task.type) {
			...
			task.type = entry ? entry.label : "General";
			delete task.color;
		}
		if (task.type === undefined || task.type === "") task.type = "General";
		...
		// Add actualTime field for tracking actual completion time
		// Only count tasks where user explicitly tracked time via completion modal
		if (task.actualTime === undefined) task.actualTime = "";
		// Parse actualTime if it exists
		if (task.actualTime) task.actualTime = parseTimeToMinutes(task.actualTime);
		// Add streak counter for repeating tasks
...
function prioritySort(sortBy) {
	return function(a, b) {
		...
		if (sortBy === "priority") {
			...
			// If same urgency coefficient, boost short tasks (< 15 min) to top, then use custom order
			if (effectivePriorityA === effectivePriorityB) {
				const aIsShort = a.length && a.length < 15;
				const bIsShort = b.length && b.length < 15;

				// If one is short and the other isn't, prioritize the short one
				if (aIsShort && !bIsShort) return -1;
				if (!aIsShort && bIsShort) return 1;

				// Otherwise use custom order as tiebreaker
				return (a.order ?? 0) - (b.order ?? 0);
			}
	...
	return list
		.map((task) => {
			...
			return {
				...
				actualTime: typeof task.actualTime === "number" ? task.actualTime : (task.actualTime || ""),
